...
package org.quiltmc.example.loader.plugin.lib;
...
public final class JsonReader implements Closeable {
	...
	public static JsonReader json5(Path in) throws IOException {
		return new JsonReader(in, JsonFormat.JSON5);
	}
	...
	public static JsonReader json(Path in) throws IOException {
		return new JsonReader(in, JsonFormat.JSON);
	}
	...
	public static JsonReader create(Path in, JsonFormat format) throws IOException {
		return new JsonReader(in, format);
	}

	public static JsonReader create(String in, JsonFormat format) {
		return new JsonReader(in, format);
	}

	public static JsonReader create(Reader in, JsonFormat format) {
		return new JsonReader(in, format);
	}

	private JsonReader(String in, JsonFormat format) {
		this(new StringReader(Objects.requireNonNull(in, "Input string cannot be null")), format);
	}

	private JsonReader(Path in, JsonFormat format) throws IOException {
		this(Files.newBufferedReader(Objects.requireNonNull(in, "Path cannot be null")), format);
	}

	private JsonReader(Reader in, JsonFormat format) {
	...
	/**
	 * Returns true if a non-execute prefix will be ignored at the beginning of this file. Defaults to false.
	 */
		...
		if (peekStack == JsonScope.EMPTY_ARRAY) {
			...
			switch (c) {
				...
				case '}':
					...
					return peeked = PEEKED_END_OBJECT;
				default:
					assertJson5(); // Unquoted names are only allowed in JSON5
	...
	private int peekNumber() throws IOException {
		...
		for (; true; i++) {
			...
			switch (c) {
				...
				case 'N':
					// NaN json5 only
					assertJson5();
					if ((last == NUMBER_CHAR_NONE) && literal(buffer, p + i, "NaN", true)) {
				...
				case 'I':
					// Infinity json5 only
					assertJson5();
					if ((last == NUMBER_CHAR_NONE || last == NUMBER_CHAR_SIGN) && literal(buffer, p + i, "Infinity", true)) {
				...
				case '+':
					...
					if (isJson5(last == NUMBER_CHAR_NONE)) {
						continue;
					}
				...
				case 'E':
					...
					if (last == NUMBER_CHAR_ZERO || last == NUMBER_CHAR_DIGIT || last == NUMBER_CHAR_FRACTION_DIGIT || isJson5(last == NUMBER_CHAR_DECIMAL)) {
				...
				case '.':
					if (last == NUMBER_CHAR_DIGIT || last == NUMBER_CHAR_ZERO || isJson5(last == NUMBER_CHAR_NONE) || isJson5(last == NUMBER_CHAR_SIGN)) {
		...
		if (last == NUMBER_CHAR_ZERO || last == NUMBER_CHAR_DIGIT || last == NUMBER_CHAR_FRACTION_DIGIT
				|| last == NUMBER_CHAR_EXP_DIGIT || isJson5(last == NUMBER_CHAR_DECIMAL)) { // JSON5 allows trailing decimal
			peekedNumberLength = i;
	...
	private boolean literal(char[] chars, int idx, String text, boolean thenEnd) throws IOException {
		...
		return !thenEnd || !isLiteral(chars[idx + text.length()]);
	}
...
//						checkNotStrict(); // fall-through
	...
	private void assertJsonc() throws IOException {
		if (this.format != JsonFormat.JSONC && this.format != JsonFormat.JSON5) {
			throw syntaxError("Invalid syntax found for parsing mode " + this.format + ", but it is valid  inJSONC or JSON5");
		}
	}

	private void assertJsonc(boolean bool) throws IOException {
		if (bool && this.format != JsonFormat.JSONC && this.format != JsonFormat.JSON5) {
			throw syntaxError("Invalid syntax found for parsing mode " + this.format + ", but it is valid  inJSONC or JSON5");
		}
	}

	private void assertJson5() throws IOException {
		if (this.format != JsonFormat.JSON5) {
			throw syntaxError("Invalid syntax found for parsing mode " + format + ", but it is valid in JSON5");
		}
	}

	private boolean isJson5(boolean bool) throws IOException {
		if (bool && this.format != JsonFormat.JSON5) {
			throw syntaxError("Invalid syntax found for parsing mode " + format + ", but it is valid in JSON5");
		}
  ...
  public String getPath() {
	...
	private IOException syntaxError(String message) throws IOException {
		throw new IOException(String.format("%s %s", message, path()));
	}
